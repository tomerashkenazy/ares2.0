...
# gradient teachers
from ares.utils.loss import build_loss, build_loss_scaler, resolve_amp
...
def get_args_parser():
    ...
    #* distributed setting
    parser.add_argument('--distributed', default=True, help='if distributed training')
    ...
    #* model parameters
    parser.add_argument('--model', default='convnext_small', type=str, metavar='MODEL', help='Name of model to train')
    ...
    #* dataset parameters
    parser.add_argument('--batch-size', default=16, type=int)    # batch size per gpu
    parser.add_argument('--grad-accum', default=1, type=int)    # gradient acumulation
    parser.add_argument('--train-dir', default='/mnt/data/datasets/imagenet_sample/train', type=str, help='train dataset path')
    parser.add_argument('--eval-dir', default='/mnt/data/datasets/imagenet_sample/val', type=str, help='validation dataset path')
    ...
    parser.add_argument('--mixup-mode', type=str, default='elem',
    ...
    parser.add_argument('--mixup_active', type=bool, default=True)
    # drop connection
    ...
    # gradnorm
    parser.add_argument('--gradnorm', default=True, help='if use contourtrain')
...
def main(args, args_text):
    ...
    # Ensure distributed initialization arguments are set
    if args.distributed:
        if not hasattr(args, 'dist_backend') or not args.dist_backend:
            args.dist_backend = 'nccl'  # Default backend for distributed training
        if not hasattr(args, 'dist_url') or not args.dist_url:
            args.dist_url = 'env://'
        if not hasattr(args, 'world_size') or not args.world_size:
            args.world_size = 1  # Default to single process

    distributed_init(args)

    # Log distributed initialization status
    if args.distributed:
        print(f"Distributed initialized: rank={args.rank}, world_size={args.world_size}, backend={args.dist_backend}")

    # Ensure output_dir is valid and writable
    if not args.output_dir or args.output_dir.startswith('/'):
    ...
    if args.rank == 0 and not os.path.exists(args.output_dir):
        ...
        # Ensure the directory for gradnorms.txt exists before opening the file
    ...
    _logger = setup_logger(args.output_dir, distributed_rank=args.rank, name='main_train', default_level=logging.INFO)
    ...
    if args.distributed:
        try:
            ...
            _logger.warning("Apex is not installed. Falling back to native Torch DistributedDataParallel.")
            model = NativeDDP(model, device_ids=[args.device_id])
        # NOTE: EMA model does not need to be wrapped by DDP

    # Ensure distributed initialization is called properly
    if args.distributed and not torch.distributed.is_initialized():
        raise RuntimeError("Distributed process group is not initialized. Ensure 'distributed_init(args)' is called and arguments are set correctly.")
    ...
    print(f'Train loss function: {train_loss_fn}')
    print(f'Validate loss function: {validate_loss_fn}')

    # setup reg loss fncs
    reg_loss_fn = []  # to be compatible with later code
    if args.advtrain:
        reg_loss_fn = None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # one epoch training
        print(f'Start training epoch {epoch} with optimizer: {optimizer}\n train_loss_fn: {train_loss_fn}\n'
              f'reg_loss_fn: {reg_loss_fn} \n lr_scheduler: {lr_scheduler}\n saver: {saver}\n amp_autocast: {amp_autocast}\n'
              f'loss_scaler: {loss_scaler}')
        train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args,
                reg_loss_fn=reg_loss_fn,
                lr_scheduler=lr_scheduler, saver=saver, amp_autocast=amp_autocast,
                loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=None, _logger=_logger)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if (batch_idx + 1) % args.grad_accum == 0:
            ...
            print(f'batch_time: {end - start_time:.4f}s')
            # end for
